...
export const getStockAdjustments = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    `;
    const params: any[] = [];
    if (product_id) { sql += ` AND sa.product_id=$1`; params.push(product_id); }
...
export const createStockAdjustment = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    await client.query('UPDATE products SET quantity_on_hand=$1, updated_at=NOW() WHERE id=$2', [newQty, product_id]);
    const adj = await client.query(
      `INSERT INTO stock_adjustments (product_id, quantity_changed, adjustment_type, reason, user_id) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [product_id, quantity_changed, adjustment_type, reason || null, req.user?.id]
    );
