...
export const getSuppliers = async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
...
export const updateSupplier = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const result = await query(
      'UPDATE suppliers SET name=COALESCE($1,name),contact_name=COALESCE($2,contact_name),email=COALESCE($3,email),phone=COALESCE($4,phone),address=COALESCE($5,address),updated_at=NOW() WHERE id=$6 RETURNING *',
...
export const getPurchaseOrders = async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await query(`
...
export const createPurchaseOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const po = await client.query(
      'INSERT INTO purchase_orders (supplier_id, total_amount, notes, created_by) VALUES ($1,$2,$3,$4) RETURNING *',
      [supplier_id, total, notes||null, req.user?.id]
    );
...
export const updatePurchaseOrderStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const po = await client.query(
      `UPDATE purchase_orders SET status=$1, order_date=COALESCE($2,order_date), received_date=COALESCE($3,received_date), updated_at=NOW() WHERE id=$4 RETURNING *`,
      [status, updateFields.order_date||null, updateFields.received_date||null, id]
    );
    ...
    if (status === 'received') {
      ...
      for (const item of items.rows) {
        await client.query('UPDATE products SET quantity_on_hand=quantity_on_hand+$1, updated_at=NOW() WHERE id=$2', [item.quantity, item.product_id]);
        await client.query(
          `INSERT INTO stock_adjustments (product_id, quantity_changed, adjustment_type, reason, user_id) VALUES ($1,$2,'receiving',$3,$4)`,
          [item.product_id, item.quantity, `PO #${id} received`, req.user?.id]
        );
      }
    }
