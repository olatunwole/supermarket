...
export const updateUser = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const result = await query(
      'UPDATE users SET email=COALESCE($1,email), role=COALESCE($2,role), is_active=COALESCE($3,is_active), updated_at=NOW() WHERE id=$4 RETURNING id, username, email, role, is_active',
